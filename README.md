...
The pre-old-phone roadmap foundation now includes a controlled external-worker registry, common adapter contract, dynamic OpenCode model discovery, local-first worker routing, bounded Goal/Work DAGs, approval-delegated Git worktree plans, structured worker messages, and least-context sharing. Codex, Hermes, Aider, and OpenCode were detected at verification time; their execution remains disabled inside this new layer. Munder and other optional workers degrade honestly. Provider-managed/cloud worker routes are off by default. Phase 7, Roadmap Step 21, and old-phone work have not started. See [Pre-Old-Phone Foundation](docs/PRE_OLD_PHONE_FOUNDATION.md).

Phase 6 is implemented as a controlled local-first tooling and intelligence layer. A post-Phase-6 repair fixes false planning classification for normal free-form prompts, adds an official-SDK local stdio MCP client, registers Playwright MCP for isolated discovery only, and distinguishes machine-installed tools from JARVIS-integrated and execution-authorized tools. Optional external adapters remain disabled unless their real adapter, policy, and approval path are configured. See [Phase 6 Operations](docs/PRODUCTION_RUNBOOK.md), [Phase 6 Commands](docs/COMMANDS.md), and [Post-Phase-6 Runtime Repair](docs/POST_PHASE6_RUNTIME_REPAIR.md).
...
Prompt 76 adds a secure MCP registry/runtime foundation. The local configuration now registers Playwright MCP with discovery-only trust and explicit isolated/headless flags. Official-SDK stdio handshake and discovery work from an already cached package; all discovered tools remain untrusted and disabled, while remote MCP, installation, scheduled execution, and global tool execution stay disabled.
...
JARVIS OS is a modular local-first AI assistant foundation written in Python 3.12+. It includes verified local Ollama chat, governed provider routing, safe tools, advisory planning, a plan-only Multi-Agent OS foundation, context and goal intelligence, an explicit Vosk push-to-talk workflow, interruptible Windows SAPI voice output, local Ollama LLaVA image analysis, partial Online Sync, bounded read-only public page inspection, planning-only Mobile Automation, and governed image/video workflow foundations. On the current repaired core environment, Vosk is detected only in an isolated tool environment and is reported as degraded until the core process can import its STT and microphone dependencies. Encrypted remote sync, interactive browsing, wake word, continuous conversation, live camera vision, live phone control, external plugins, and autonomous specialist execution are not implemented by default.
...
Use `web open <https-url>` for bounded public HTML/text inspection, then `web title`, `web url`, or `web snapshot`. This does not launch or control a browser; clicking, typing, forms, login, downloads, and account actions remain blocked.
