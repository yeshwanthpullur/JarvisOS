    ...
    def parse(self, text: str) -> ParsedCommand:
        ...
        for part in parts[1:]:
            if part.startswith("--"):
                ...
                args.append(part)
        name = parts[0]
        if args and f"{name} {args[0]}" in {"provider list", "provider status", "provider health", "plugin list", "plugin status", "agent list", "agent status", "department list", "memory search", "knowledge search", "task list", "task status", "workflow list", "config show", "logs recent", "profile show", "profile list", "profile explain", "profile update", "profile forget", "profile confirm", "profile reject", "context show", "context recent", "context clear", "context pause", "context resume", "context previous", "objective show", "goal show", "goal list", "goal review", "goal progress", "goal next", "goal blockers", "goal evaluate", "goal conflicts", "goal align", "goal portfolio", "goal pause", "goal resume", "goal complete", "local status", "local providers", "local models", "local refresh", "local use", "local test", "local explain-selection", "local only"}:
