...
def _workflow_runtime(context: CommandContext) -> WorkflowRuntime:
    conversation = context.conversation_context
...
def _telegram_runtime(context: CommandContext) -> TelegramRuntime:
    conversation=context.conversation_context
    direct=getattr(conversation,"telegram_runtime",None) if conversation is not None else None
    metadata=getattr(conversation,"metadata",{}) or {} if conversation is not None else {}
    runtime=direct if isinstance(direct,TelegramRuntime) else metadata.get("telegram_runtime")
    if isinstance(runtime,TelegramRuntime):return runtime
    runtime=TelegramRuntime()
    if conversation is not None:
        try:setattr(conversation,"telegram_runtime",runtime)
...
def _github_provider(context:CommandContext):
    conversation=context.conversation_context;runtime=getattr(conversation,"github_provider",None) if conversation is not None else None
    if isinstance(runtime,GitHubProvider):return runtime
    runtime=GitHubProvider()
    if conversation is not None:
        try:setattr(conversation,"github_provider",runtime)
        except Exception:pass
    return runtime
...
def register_builtin_commands(registry: CommandRegistry) -> None:
    ...
    commands: tuple[tuple[str, str, str, tuple[str, ...], CommandPermission], ...] = (
        ...
        *((name, "Phase 6 observability command", "diagnostic", (), CommandPermission.DIAGNOSTIC) for name in ("performance status","performance startup","performance providers","performance models","performance memory","performance voice","operations status","operations alerts","operations audit","phase6 status","phase6 candidate","phase6 checklist")),
        ...
        ("research provider-health", "Show bounded provider health", "research", (), CommandPermission.DIAGNOSTIC),
        ("research budget", "Show finite research mode budgets", "research", (), CommandPermission.DIAGNOSTIC),
        ...
        ("research search", "Run configured bounded read-only research", "research", (), CommandPermission.DIAGNOSTIC),
        ("research sources", "Show possible source policy", "research", (), CommandPermission.DIAGNOSTIC),
        ("research quick", "Run a bounded quick research request", "research", (), CommandPermission.DIAGNOSTIC),
        ("research standard", "Run a bounded standard research request", "research", (), CommandPermission.DIAGNOSTIC),
        ("research deep", "Run a bounded deep research request", "research", (), CommandPermission.DIAGNOSTIC),
        ("research verify", "Verify a claim using configured evidence", "research", (), CommandPermission.DIAGNOSTIC),
        ...
        ("research contradictions", "Show bounded contradiction metadata", "research", (), CommandPermission.DIAGNOSTIC),
        ("research citations", "Show verified source references", "research", (), CommandPermission.DIAGNOSTIC),
