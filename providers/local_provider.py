"""Local OpenAI-compatible provider integration."""
...
class LocalProvider(BaseProvider):
    """Provider contract for a trusted local OpenAI-compatible runtime."""
    ...
    def health_check(self) -> ProviderHealth:
        ...
        try:
            ...
            if reachable:
                ...
                self.health.mark_failure("Local provider endpoint is unreachable.")
    ...
    def _execute_chat(self, request: ProviderRequest) -> ProviderResponse:
        ...
        if model and models and model not in {item.model_id for item in models}:
            return self._error_response(request, model, "Model missing: the requested local model is not available.", start)
        if not model:
            model = self._first_model_id()
        if not model:
            return self._error_response(request, "", "No usable local models were found.", start)
    ...
    def _discover_models(self) -> tuple[ModelInfo, ...]:
        try:
            raw = self._request_json(f"{self._base_url()}/v1/models", timeout=self.context.config.timeout_seconds or 30)
        except Exception:
        ...
        for item in data:
            ...
            models.append(
                ModelInfo(
                    ...
                    context_window=int(item.get("context_window") or item.get("max_context") or 0),
                    max_tokens=int(item.get("max_tokens") or 0),
                    metadata={"source": "openai-compatible", "raw": item},
                )
            )
    ...
    def _ping(self) -> bool:
        try:
            self._request_json(f"{self._base_url()}/v1/models", timeout=min(5, self.context.config.timeout_seconds or 5))
            return True
        except Exception:
            return False
    ...
    def _normalize_chat_response(self, raw: dict[str, Any]) -> tuple[str, ProviderUsage, str | None]:
        ...
        if not choices:
            raise RuntimeError("Malformed local response: no choices returned.")
