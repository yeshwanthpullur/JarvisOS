...
Playwright MCP is discovery-only. It uses an isolated headless context, does not save a session, receives no browser-profile path, and has no trusted tools. `mcp start` performs handshake plus metadata discovery, while explicit `mcp discover` refreshes that metadata; neither executes a tool. Shutdown always stops the local process.
