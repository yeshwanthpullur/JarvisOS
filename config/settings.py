...
def load_settings(
    ...
    return AppSettings(
        ...
        vision=VisionConfig(**{**raw_config["vision"],"enabled":_coerce_bool(raw_config["vision"]["enabled"]),"local_only":_coerce_bool(raw_config["vision"]["local_only"]),"allowed_directories":tuple(_resolve_path(x) for x in raw_config["vision"]["allowed_directories"])}),
        sync=SyncConfig(**{
            **raw_config["sync"],
