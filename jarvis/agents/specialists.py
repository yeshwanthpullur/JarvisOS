...
def specialist_entries(*, vision_ready: bool = False) -> tuple[E, ...]:
    current = (
        ...
        _entry("research_agent", "Read-only research planning and bounded evidence summaries.", (_cap("research", T.RESEARCH, "Research planning, evidence requirements, and bounded summaries.", limitations=("Read-only and plan-first; evidence retrieval is limited when read-only web access is unavailable.",)),_cap("github_repository_read",T.RESEARCH,"Bounded GitHub metadata is available only when the scoped provider is ready.",permissions=("github_read",))), ready=True, health="partial_research", reason="Research planning is ready; source retrieval remains bounded by read-only web, document, and scoped GitHub provider availability."),
