...
def specialist_entries(*, vision_ready: bool = False) -> tuple[E, ...]:
    current = (
        ...
        _entry("research_agent", "Evidence-driven bounded external research runtime.", (_cap("research", T.RESEARCH, "Quick, standard, and deep research planning with provenance, citations, contradiction and freshness metadata.", limitations=("Live retrieval requires an explicitly configured governed read-only provider.",)),_cap("github_repository_read",T.RESEARCH,"Bounded GitHub metadata is available only when the scoped provider is ready.",permissions=("github_read",))), ready=True, health="partial_research", reason="The evidence runtime is implemented; live external search remains unconfigured by default and research has no execution or memory-write authority."),
        _entry("coding_agent", "Read-only repository diagnostics and plan-only coding support.", (
            ...
            _cap("github_service",T.CODING,"Scoped GitHub reads and approval-gated issue/PR/release service actions.",risk=R.HIGH,approval=True,permissions=("github_read","github_write"),limitations=("Local Git remains authoritative; merge, workflow execution, secrets, and administration are blocked.",)),
    ...
    future = (
        _entry("mcp_gateway_agent", "Controlled MCP registry, discovery, resources, and execution planning.", (_cap("tools", T.TOOLS, "Configured MCP discovery and trusted resource reads; side effects remain approval/Broker controlled.", risk=R.HIGH, approval=True, permissions=("mcp_discover","mcp_read","mcp_execute"), limitations=("No installation, arbitrary server, remote default, scheduled execution, shell, or unrestricted filesystem.",)),), ready=True, health="controlled_foundation", reason="MCP runtime foundation is implemented; no servers are registered or trusted by default."),
