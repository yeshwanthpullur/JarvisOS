...
def render_runtime_command(runtime:ReliabilityRuntime,command:str,args:tuple[str,...])->str:
 try:
  ...
  if command=="runtime breakers":return "Runtime breakers: "+"; ".join(f"{k}:{v.state.value}:{v.failure_count}" for k,v in sorted(runtime.breakers.items()))
  ...
  return "Runtime commands: status, health, components, dependencies, metrics, diagnostics, alerts, providers, models, queues, resources, breakers, recovery-plan, recovery-history, events, dashboard, profile, capacity, verify."
