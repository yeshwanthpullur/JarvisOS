...
class ProjectTrackingTests(unittest.TestCase):
    ...
    def test_project_health_json_is_valid_and_bounded(self) -> None:
        ...
        self.assertEqual(health["next_milestone"], "Phase 3 Batch 2 complete; awaiting release approval")
        self.assertEqual(health["overall_mvp_readiness"], 87)
    ...
    def test_project_status_command_is_concise_and_data_backed(self) -> None:
        ...
        self.assertIn("MVP=87%", response.response)
        self.assertIn("Phase 3 Batch 2 complete", response.response)
        ...
        self.assertEqual(response.metadata["overall_mvp_readiness"], 87)
